<?php
/**
 * Content Warning custom taxonomy archives
 *
 * @package WordPress
          ...
          <div class="tax-cloud__header">
            <h1 class="tax-cloud__current">
              <?php
                printf(
                  _n(
                    '<span class="tax-cloud__number">%1$s</span> Result with the <em>"%2$s"</em> warning' . $parent_snippet,
                    '<span class="tax-cloud__number">%1$s</span> Results with the <em>"%2$s"</em> warning' . $parent_snippet,
                    $current_count,
                    'fictioneer'
                  ),
                  $current_count,
...
<?php
  ...
  $footer_args = array(
    'post_type' => null,
    'post_id' => null,
    'template' => 'taxonomy-fcn_content_warning.php',
    'breadcrumbs' => array(
      [fcntr( 'frontpage' ), get_home_url()],
      [sprintf( __( 'Results for Content Warning "%s"', 'fictioneer' ), single_tag_title( '', false ) ), null]
    )
  );
