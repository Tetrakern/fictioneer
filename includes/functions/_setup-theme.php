...
/**
 * Authenticate REST API request to patch a story.
 *
 * @since 5.29.5
 *
...
function fictioneer_rest_story_auth_callback( $post_id, $user_id ) {
  ...
  if ( $post ) {
    return (
      ...
      user_can( $user_id, 'edit_others_fcn_stories' ) ||
      user_can( $user_id, 'manage_options' )
    );
  }

  return user_can( $user_id, 'edit_fcn_stories' ) || user_can( $user_id, 'manage_options' );
}

/**
 * Register theme meta fields with WordPress.
 *
 * Note: This may also need you to enable custom field support
 * for custom post types in the settings.
 *
 * @since 5.29.5
 */

function fictioneer_register_meta_fields() {
  register_post_meta(
    'fcn_story',
  ...
  register_post_meta(
    ...
    array(
      ...
      'sanitize_callback' => function( $meta_value ) {
        ...
        $meta_value = array_filter( $meta_value, function( $value ) { return $value > 0; } );
      ...
      'sanitize_callback' => function( $meta_value ) {
        ...
        $meta_value = array_map(
          function( $slide ) {
            ...
            $chapter_ids = array_filter( $chapter_ids, fn( $id ) => $id >= 1 );
...
add_action( 'init', 'fictioneer_register_meta_fields' );
